...
public class CraftDataTable
{
	...
	public String getDataForTRNid(String datasheetName, String fieldName)
	{
		checkPreRequisites();
	...
	public String getDataAll(String datasheetName, String fieldName)
    {
           checkPreRequisites();
           String dataValue = "";
           ExcelDataAccess testDataAccess = new ExcelDataAccess(datatablePath, datatableName);
           ...
           int nGetInstances = testDataAccess.getLastRowNum();
           
           int rowNum = testDataAccess.getRowNum(currentTestcase, 0, 1); // Start at row 1, skipping the header row
           for (int currentGetInstance = rowNum; currentGetInstance <= nGetInstances; currentGetInstance++) {
                  String strCurrentTestcase = testDataAccess.getValue(currentGetInstance, "TC_ID");
                  String strCurrentIteration = testDataAccess.getValue(currentGetInstance, "Iteration");
                  if(strCurrentTestcase.equalsIgnoreCase(currentTestcase)){
                        if(strCurrentIteration.equalsIgnoreCase(Integer.toString(currentIteration))){
                               
                               
                             dataValue = dataValue + ";" + testDataAccess.getValue(currentGetInstance, fieldName);
	...
	public String getDataSubIter(String datasheetName, String fieldName, int subIteration)
    {
           checkPreRequisites();
           String dataValue = "";
           ExcelDataAccess testDataAccess = new ExcelDataAccess(datatablePath, datatableName);
           ...
           for (int currentGetInstance = rowNum; currentGetInstance <= nGetInstances; currentGetInstance++) {
                  String strCurrentTestcase = testDataAccess.getValue(currentGetInstance, "TC_ID");
                  String strCurrentIteration = testDataAccess.getValue(currentGetInstance, "Iteration");
                  String strCurrentSubIteration = testDataAccess.getValue(currentGetInstance, "SubIteration");
                  if(strCurrentTestcase.equalsIgnoreCase(currentTestcase)){
                        if(strCurrentIteration.equalsIgnoreCase(Integer.toString(currentIteration))){
                           //String strCurrentSubIteration = testDataAccess.getValue(currentGetInstance, "SubIteration");     
                               if(strCurrentSubIteration.equalsIgnoreCase(Integer.toString(subIteration))){
                               
                                      dataValue = testDataAccess.getValue(currentGetInstance, fieldName);
                                      break;
                               }
                        }
                  }
	...
	public void putDataAll(String datasheetName, String fieldName, String dataValue)
	{
		checkPreRequisites();
		...
		int rowNum = testDataAccess.getRowNum(currentTestcase, 0, 1);	// Start at row 1, skipping the header row
		for (int currentPutInstance = rowNum; currentPutInstance <= nPutInstances; currentPutInstance++) {
			String strCurrentTestcase = testDataAccess.getValue(currentPutInstance, "TC_ID");
			String strCurrentIteration = testDataAccess.getValue(currentPutInstance, "Iteration");
			if(strCurrentTestcase.equalsIgnoreCase(currentTestcase)){
				if(strCurrentIteration.equalsIgnoreCase(Integer.toString(currentIteration))){
	...
	public void putDataSubIter(String datasheetName, String fieldName, String dataValue, int subIteration)
    {
           checkPreRequisites();
           ...
           rowNum = testDataAccess.getRowNum(Integer.toString(currentIteration), 1, rowNum);
           if (rowNum == -1) {
                  throw new FrameworkException("The iteration number \"" + subIteration + "\"" +
                                                                  "of the test case \"" + currentTestcase + "\"" +
                                                                  "is not found in the test data sheet \"" + datasheetName + "\"!");
           }
           rowNum = testDataAccess.getRowNum(Integer.toString(subIteration), 2, rowNum);
           if (rowNum == -1) {
                  throw new FrameworkException("The sub iteration number \"" + subIteration + "\"" +
                                                                  "under iteration number \"" + currentIteration + "\"" +
                                                                  "of the test case \"" + currentTestcase + "\"" +
                                                                  "is not found in the test data sheet \"" + datasheetName + "\"!");
           }
